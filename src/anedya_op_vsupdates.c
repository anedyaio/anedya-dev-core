...
anedya_err_t _anedya_parse_valuestore_string(char *payload, size_t payload_len, anedya_valuestore_obj_string_t *obj)
{
    ...
    if (!json)
    {
        _anedya_interface_std_out("Error while parsing JSON body: Valuestore string");
        return ANEDYA_ERR;
    }
    ...
    jsonType_t val_type = json_getType(value);
    // printf("Property: %d", val_type);
    if (!value || (JSON_REAL != val_type && JSON_TEXT != val_type))
    {
        _anedya_interface_std_out("Error, the value property is not found.");
        return ANEDYA_ERR;
    }
    if (val_type == JSON_TEXT)
    {
        obj->value = (char *)json_getValue(value);
    }
    else
    {
        _anedya_interface_std_out("Error, unsupported value type.");
        return ANEDYA_ERR;
    }
...
anedya_err_t _anedya_parse_valuestore_bool(char *payload, size_t payload_len, anedya_valuestore_obj_bool_t *obj)
{
    ...
    if (!json)
    {
        _anedya_interface_std_out("Error while parsing JSON body: Valuestore boolean");
        return ANEDYA_ERR;
    }
    ...
    if (val_type == JSON_BOOLEAN)
    {
        obj->value = (bool)json_getBoolean(value);
    }
...
anedya_err_t _anedya_parse_valuestore_bin(char *payload, size_t payload_len, anedya_valuestore_obj_bin_t *obj)
{
    ...
    if (!json)
    {
        _anedya_interface_std_out("Error while parsing JSON body: Valuestore binary");
        return ANEDYA_ERR;
    }
