...
// First check if client is already connected or not
  ...
  int resp_len = 0;
  err = _anedya_interface_http_post(client, "/v1/commands/next", txbuffer,
...
void _anedya_op_cmd_handle_next_resp(anedya_client_t *client,
                                     anedya_txn_t *txn) {
  ...
  if (!status_prop || JSON_TEXT != json_getType(status_prop)) {
    _anedya_interface_std_out(
        "Error, the status property is not found or is not text.");
    return;
  }
  ...
  if (!data_prop || JSON_TEXT != json_getType(data_prop)) {
    _anedya_interface_std_out(
        "Error, the data property is not found or is not text.");
    return;
  }
  ...
  if (!datatype_prop || JSON_TEXT != json_getType(datatype_prop)) {
    _anedya_interface_std_out(
        "Error, the datatype property is not found or is not text.");
    return;
  }
  ...
  if (!issued_at_prop || JSON_INTEGER != json_getType(issued_at_prop)) {
    _anedya_interface_std_out(
        "Error, the issued at property is not found or is not text.");
    return;
  }
  ...
  if (!updated_prop || JSON_INTEGER != json_getType(updated_prop)) {
    _anedya_interface_std_out(
        "Error, the updated property is not found or is not text.");
    return;
  }
  ...
  if (!nextavailable_prop || JSON_BOOLEAN != json_getType(nextavailable_prop)) {
    _anedya_interface_std_out(
        "Error, the next available property is not found or is not text.");
    return;
  }
