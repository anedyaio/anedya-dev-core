...
// First check if client is already connected or not
  ...
  int resp_len = 0;
  err = _anedya_interface_http_post(client, "/v1/commands/list", txbuffer,
...
void _anedya_op_command_handle_list_resp(anedya_client_t *client,
                                         anedya_txn_t *txn) {
  ...
  anedya_op_cmd_list_resp_t *resp = (anedya_op_cmd_list_resp_t *)txn->response;
  ...
  while (cmds_prop && i < resp->count) {
    ...
    snprintf(resp->commands[i].command, sizeof(resp->commands[i].command), "%s",
             cmd);
    ...
    snprintf(resp->commands[i].status, sizeof(resp->commands[i].status), "%s",
             status);
    ...
    resp->commands[i].issued_at =
        (unsigned long long)json_getInteger(issued_at_prop);
    ...
    resp->commands[i].updated =
        (unsigned long long)json_getInteger(updated_prop);

    cmds_prop =
        json_getSibling(cmds_prop); // Move to the next sibling in the array
    i++;
  }
