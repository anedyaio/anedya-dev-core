...
// First check if client is already connected or not
  ...
  int resp_len = 0;
  err = _anedya_interface_http_post(client, "/v1/valuestore/getValue", txbuffer,
...
void _anedya_op_valuestore_handle_get_resp(anedya_client_t *client,
                                           anedya_txn_t *txn) {
  // printf("Resp: %s\n", txn->_rxbody);
  json_t mem[32];
  ...
  if (!json) {
    _anedya_interface_std_out(
        "Error while parsing JSON body:response handler Get value store");
    return;
  }
  // Check if success
  ...
  if (strcmp(t, "string") == 0) {
    anedya_valuestore_obj_string_t *resp =
        (anedya_valuestore_obj_string_t *)txn->response;
    ...
    anedya_valuestore_obj_float_t *resp =
        (anedya_valuestore_obj_float_t *)txn->response;
    json_t const *scope = json_getProperty(namespace, "scope");
    if (!scope || JSON_TEXT != json_getType(scope)) {
      _anedya_interface_std_out("Error, the scope property is not found.");
      return;
    ...
    anedya_valuestore_obj_bin_t *resp =
        (anedya_valuestore_obj_bin_t *)txn->response;
    json_t const *scope = json_getProperty(namespace, "scope");
    if (!scope || JSON_TEXT != json_getType(scope)) {
      _anedya_interface_std_out("Error, the scope property is not found.");
      return;
    ...
    anedya_valuestore_obj_bool_t *resp =
        (anedya_valuestore_obj_bool_t *)txn->response;
    json_t const *scope = json_getProperty(namespace, "scope");
    if (!scope || JSON_TEXT != json_getType(scope)) {
      _anedya_interface_std_out("Error, the scope property is not found.");
      return;
