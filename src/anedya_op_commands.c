...
anedya_err_t _anedya_parse_inbound_command(char *payload,
                                           unsigned int payload_len,
                                           anedya_command_obj_t *obj) {
  json_t mem[32];
  char *temp = malloc(payload_len + 1);
  if (!temp) return ANEDYA_ERR_NO_MEMORY;
  ...
  if (!json) {
    _anedya_interface_std_out("Error while parsing JSON body: Command");
    free(temp);
    return ANEDYA_ERR_PARSE_ERROR;
  }
  json_t const *cmdID = json_getProperty(json, "commandId");
  if (!cmdID || JSON_TEXT != json_getType(cmdID)) {
    _anedya_interface_std_out("Error, the commandId property is not found.");
    free(temp);
    return ANEDYA_ERR_PARSE_ERROR;
  }
  ...
  if (!cmd || JSON_TEXT != json_getType(cmd)) {
    _anedya_interface_std_out("Error, the command property is not found.");
    free(temp);
    return ANEDYA_ERR_PARSE_ERROR;
  }
  ...
  if (!dt || JSON_TEXT != json_getType(dt)) {
    _anedya_interface_std_out("Error, the datatype property is not found.");
    free(temp);
    return ANEDYA_ERR_PARSE_ERROR;
  }
  ...
  if (_anedya_strcmp(dt_val, "string") == 0) {
    ...
    if (!data || JSON_TEXT != json_getType(data)) {
      _anedya_interface_std_out(
          "Error, the data property is not found.");
      free(temp);
      return ANEDYA_ERR_PARSE_ERROR;
    }
  ...
  if (_anedya_strcmp(dt_val, "binary") == 0) {
    ...
    obj->data_len =
        _anedya_base64_decode((unsigned char *)data_content, obj->data);
  ...
  if (!exp || JSON_INTEGER != json_getType(exp)) {
    _anedya_interface_std_out("Error, the exp property is not found.");
    free(temp);
    return ANEDYA_ERR_PARSE_ERROR;
  }
...
// TODO: Implement dynamic allocation
  ...
  int resp_len = 0;
  err = _anedya_interface_http_post(client, "/v1/commands/updateStatus",
                                    txbuffer, (int)strlen(txbuffer), resp_buf,
