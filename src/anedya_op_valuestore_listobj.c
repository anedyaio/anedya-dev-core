...
// First check if client is already connected or not
  ...
  int resp_len = 0;
  err = _anedya_interface_http_post(client, "/v1/valuestore/scan", txbuffer,
...
void _anedya_op_valuestore_handle_list_obj_resp(anedya_client_t *client,
                                                anedya_txn_t *txn) {
  ...
  if (!json) {
    _anedya_interface_std_out(
        "Error while parsing JSON body:response handler Get list value store");
    return;
  }
  // Check if success
  ...
  anedya_op_valuestore_list_obj_resp_t *resp =
      (anedya_op_valuestore_list_obj_resp_t *)txn->response;
  ...
  while (keys_prop && i < resp->count) {
    ...
    if (!namespace_prop || JSON_OBJ != json_getType(namespace_prop)) {
      _anedya_interface_std_out(
          "Error, the namespace property is not found or is not text.");
      return;
    }
    json_t const *scope_prop = json_getProperty(namespace_prop, "scope");
    if (!scope_prop || JSON_TEXT != json_getType(scope_prop)) {
      _anedya_interface_std_out(
          "Error, the scope property is not found or is not text.");
      return;
    }
    ...
    if (!id_prop || JSON_TEXT != json_getType(id_prop)) {
      _anedya_interface_std_out(
          "Error, the id property is not found or is not text.");
      const char *id = "";
    ...
    if (!key_prop || JSON_TEXT != json_getType(key_prop)) {
      _anedya_interface_std_out(
          "Error, the key property is not found or is not text.");
      return;
    }
    ...
    if (!type_prop || JSON_TEXT != json_getType(type_prop)) {
      _anedya_interface_std_out(
          "Error, the type property is not found or is not text.");
      return;
    }
    ...
    if (!modified_prop || JSON_INTEGER != json_getType(modified_prop)) {
      _anedya_interface_std_out(
          "Error, the modified property is not found or is not text.");
      return;
    }
    resp->keys[i].modified = (int64_t)json_getInteger(modified_prop);

    keys_prop =
        json_getSibling(keys_prop); // Move to the next sibling in the array
    i++;
  }
