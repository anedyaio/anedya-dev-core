...
#endif
  p = anedya_json_str(p, "key", key, &marker);
  p = anedya_json_str(p, "value", base64_value,
                      &marker); // Use the provided Base64 value
  p = anedya_json_str(p, "type", "binary",
                      &marker); // Indicate that the value is Base64-encoded
  p = anedya_json_objClose(p, &marker);
  p = anedya_json_end(p, &marker);
