...
anedya_err_t anedya_client_init(anedya_config_t *config, anedya_client_t *client)
{
    ...
    client->tx_buffer = _anedya_interface_allocate_memory(config->tx_buffer_size);
    if (client->tx_buffer == NULL)
    {
        return ANEDYA_ERR_NO_MEMORY;
    }
    client->rx_buffer = _anedya_interface_allocate_memory(config->rx_buffer_size);
    if (client->rx_buffer == NULL)
    {
        return ANEDYA_ERR_NO_MEMORY;
    }
#ifdef ANEDYA_ENABLE_DEVICE_LOGS
    client->log_buffer = _anedya_interface_allocate_memory(config->log_buffer_size * config->log_batch_size);
...
{
    anedya_err_t err;
    err = _anedya_interface_mqtt_connect(client->mqtt_client);
    if (err != ANEDYA_OK)
    {
        return err;
    }
...
anedya_err_t anedya_client_disconnect(anedya_client_t *client)
{
    ...
    err = _anedya_interface_mqtt_disconnect(client->mqtt_client);
    if (err != ANEDYA_OK)
    {
        return err;
    }
...
anedya_err_t anedya_client_destroy(anedya_client_t *client) {
    anedya_err_t err;
    err = _anedya_interface_mqtt_destroy(client->mqtt_client);
    if (err != ANEDYA_OK)
    {
        return err;
    }
...
void _anedya_on_connect_handler(anedya_client_t *client)
{
    ...
    _anedya_interface_mqtt_subscribe(client->mqtt_client, client->_message_topics[0], strlen(client->_message_topics[0]), 0);
    _anedya_interface_mqtt_subscribe(client->mqtt_client, client->_message_topics[1], strlen(client->_message_topics[1]), 0);
    _anedya_interface_mqtt_subscribe(client->mqtt_client, client->_message_topics[2], strlen(client->_message_topics[2]), 0);
    _anedya_interface_mqtt_subscribe(client->mqtt_client, client->_message_topics[3], strlen(client->_message_topics[2]), 0);
...
void _anedya_on_disconnect_handler(anedya_client_t *client)
{
    if(client->is_connected == 1) {
        ...
        // Process retry logic
        client->is_connected = 0;
        _anedya_interface_mqtt_connect(client->mqtt_client);
